...
const ContentLayout = ({ isLoading, children }: ContentLayoutProps) => {
  ...
  if (isLoading)
    return (
      <div className="flex flex-col flex-1 w-full grow max-h-full !overflow-y-auto justify-center items-center">
  ...
  if (isOnboarding)
    return (
      <div className="flex flex-col flex-1 w-full grow max-h-full !overflow-y-auto justify-center">
        {children}
      </div>
    );
  ...
  if (!isLoggedIn)
    return (
      <div className="flex flex-col flex-1 w-full grow max-h-full !overflow-y-auto">
        {children}
      </div>
    );
  ...
  return (
    <div
      className="flex flex-1 flex-col w-full grow max-h-full !overflow-y-auto relative"
      id="main-content"
    >
      {children}
    </div>
